...
/*
    1. 시퀀스 객체 생성 방법
     ...
     [CYCLE | NOCYCLE]      --> 값을 순환 여부 지정(기본값 NOCYCLE)
     [CACHE | NOCACHE]      --> 캐시메모리 할당(기본값 CACHE 20)
     
     * 캐시메모리 : 미리 발생될 값들을 생성해서 저장해두는 공간
                  매번 호출될 때마다 새롭게 번호를 생성하는게 아니라
                  캐시메모리 공간에 미리 생성된 번호가져다 쓸수 있음(속도가 빨라짐)
                  접속해제하면 캐시메모리에 미리 만들어 둔 번호들은 다 날아감
     테이블 : TB_
     뷰 : VW_
     시퀀스 : SEQ_
     트리거 : TRG_
...
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; --오류 : 지정한 MAXVALUE를 넘길 수 없음
