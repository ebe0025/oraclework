...
/*
   * 뷰컬럼에 별칭 부여
     서브쿼리의 서브쿼리에 함수식, 산술식이 기술되면 반드시 별칭을 부여해 줘야 함
*/
...
/*
    * 단, DML명령어로 조작이 불가능한 경우가 더 많음
    1) 뷰에 정의되지 않는 컬럼을 조작하고자 하는 경우
    2) 뷰에 정의되어 있는 컬럼 중에 원본테이블 상에 NOT NULL제약조건이 지정되어 있는 경우
...
-- 2) 뷰의 정의되어 있지 않은 컬럼 중에서 원본테이블 상에 NOT NULL제약조건이 지정되어 있는 경우
CREATE OR REPLACE VIEW VM_JOB
AS SELECT JOB_NAME
   FROM JOB;
...
-- 실제 원본테이블에는 (NULL, '인턴'); -- JOB_CODE는 NULL 값을 허용하지 않음
...
/*
   * VIEW 옵션
   ...
   2) FORCE | NOFORCE
      ...
      > NOFORCE : 서브쿼리에 기술된 테이블이 실제 존재해야만 뷰가 생성됨
